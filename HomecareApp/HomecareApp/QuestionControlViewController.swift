...
class QuestionControlViewController: UIViewController, SFSpeechRecognizerDelegate {
    ...
    let questionBox = QuestionBox(bodyPartToBeUsed: .ankle)
    ...
    func recordAndRecognizeSpeech() {
        ...
        recognitionTask = speechRecognizer?.recognitionTask(with: request, resultHandler: { (result, error) in
            if let result = result {
                let response = result.bestTranscription.formattedString
                let endString = response.substring(from: response.index(response.chara, offsetBy: <#T##String.IndexDistance#>))
    ...
    func updateTextLabel() {
        textLabel.text = questionBox.getCurrentQuestion().getQuestionText()
    }
